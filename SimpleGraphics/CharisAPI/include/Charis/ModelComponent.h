...
namespace Charis {
	...
	{
	public:
		/// <summary>Constructor for a ModelComponent.</summary>
		...
		ModelComponent(const float* vertexAttributes, unsigned int numberOfVertexAttributes, const std::vector<unsigned int>& floatsPerAttributePerVertex);
		/// <summary>Constructor for a ModelComponent.</summary>
		/// <param name="vertexAttributes">Vector that contains all vertices and relevant vertex attributes.</param>
		/// <param name="floatsPerAttributePerVertex">This provides a list that for each shader attribute provides the number of floats it contains. 
		/// For example, if the shaders first input is vec3 xyz and second input is vec3 rgb, then this argument should be { 3, 3 }.</param>
		ModelComponent(const std::vector<float>& vertexAttributes, const std::vector<unsigned int>& floatsPerAttributePerVertex);
		/// <summary>Constructor for a ModelComponent.</summary>
		/// <param name="vertexAttributes">Pointer to an array that contains all vertices and vertex attributes.</param>
		...
		/// <param name="floatsPerAttributePerVertex">This provides a list that for each shader attribute provides the number of floats it contains. 
		/// For example, if the shaders first input is vec3 xyz and second input is vec3 rgb, then this argument should be { 3, 3 }.</param>
		ModelComponent(const float* vertexAttributes, unsigned int numberOfVertexAttributes, const unsigned int* indices, unsigned int numberOfIndices, const std::vector<unsigned int>& floatsPerAttributePerVertex);
		/// <summary>Constructor for a ModelComponent.</summary>
		/// <param name="vertexAttributes">Vector that contains all vertices and relevant vertex attributes.</param>
